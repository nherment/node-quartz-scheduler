...
var debug = require('debug')('node-quartz-scheduler:Scheduler');
...
Scheduler.prototype.update = function(jobName, date, data, jobId, callback) {
  debug('schedule job with jobName: %s, date: %s, jobKey: %s and data:', jobName, date, jobId);
  debug(data);
  ...
  request.put({url:this._quartzURL, json: body}, function (error, response, body) {
    if(error) {
      ...
      var err = new Error('Failed to update job ['+jobName+']. Http error ['+response.statusCode+'] '+JSON.stringify(body));
      callback(err, undefined);
    } else {
      callback(undefined, body.key);
    }
